classdef approx4Controller < matlab.System
    ...
    methods(Access = protected)
        ...
        %   V_servo: voltage to the servo input.                    
            ...
            xi1 = p_ball;
            xi2 = theta;
            xi3 = (5*g)/(7*L)*r_g*sin(theta) - 5/7*(L/2-p_ball)*(r_g/L)^2*v_theta^2*cos(theta)^2;
            xi4 = v_theta*(5*g*r_g*cos(theta)/(7*L) + 2*r_g^2*v_theta^2*cos(theta)*sin(theta)*(5*L/14 - 5*p_ball/7)/L^2) + 5*r_g^2*v_theta^2*cos(theta)^2*v_ball/(7*L^2);
            psi2 = 5/(7*L^2)*r_g^2*p_ball*v_theta^2*cos(theta)^2 - 5/(14*L)*r_g^2*v_theta^2*cos(theta)^2;
            
            a = ((K*((5*g*r_g*cos(theta))/(7*L) + (2*r_g^2*v_theta^2*cos(theta)*sin(theta)*((5*L)/14 - (5*p_ball)/7))/L^2))/tau);
            b = (5*r_g^2*v_theta^2*cos(theta)^2*((5*g*r_g*sin(theta))/(7*L) - (r_g^2*cos(theta)^2*v_theta^2*((5*L)/14 - (5*p_ball)/7))/L^2))/(7*L^2) - (v_theta*((5*g*r_g*cos(theta))/(7*L) + (2*r_g^2*v_theta^2*cos(theta)*sin(theta)*((5*L)/14 - (5*p_ball)/7))/L^2))/tau - v_theta*(v_theta*((5*g*r_g*sin(theta))/(7*L) - (2*r_g^2*v_theta^2*cos(theta)^2*((5*L)/14 - (5*p_ball)/7))/L^2 + (2*r_g^2*v_theta^2*sin(theta)^2*((5*L)/14 - (5*p_ball)/7))/L^2) + (10*r_g^2*v_theta^2*cos(theta)*sin(theta)*v_ball)/(7*L^2)) - (10*r_g^2*v_theta^2*cos(theta)*sin(theta)*v_ball*v_theta)/(7*L^2);
